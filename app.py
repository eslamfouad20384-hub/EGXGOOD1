...
# 🔄 آخر شمعة تلقائيًا
# =========================================================
def get_latest_available_candle(df):
    """يرجع آخر شمعة فعلية متاحة بعد تنظيف الفهارس والقيم."""
...
def latest_candle_date(df):
    """تاريخ آخر شمعة فعلية متاحة."""
    if df is None or df.empty:
        return None
    try:
...
st.caption("🔄 تاريخ آخر شمعة يتم تحديده تلقائيًا من أحدث بيانات متاحة لكل سهم.")
st.caption(
...
def load_data(symbols, period, interval, force_refresh=False):
    _refresh_token = force_refresh
    ...
    with ThreadPoolExecutor(
        ...
        for future in as_completed(
            ...
            try:
                ...
                if result:
                    try:
                        latest_data_date = latest_candle_date(df_d)
                    except Exception:
