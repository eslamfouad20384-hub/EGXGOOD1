...
st.set_page_config(
    page_title="EGX AI PRO MAX v8.2 - عربي",
    page_icon="📈",
    layout="wide"
)

st.title("🚀 EGX AI PRO MAX v8.2")
st.caption(
...
# =========================================================
def run_real_backtest(df, max_bars=300):
    """Event-driven historical backtest using the SAME indicator engine as the scanner.

    Important v8.3 fixes:
    - The previous v8.2 passed raw OHLCV data into this function, while the
      backtest expected indicator columns (ema20, rsi, breakout, pullback, ...).
      That made every row fail the required-column check and could produce zero
      trades even when enough candles existed.
    - Indicators are now calculated here before the simulation.
    - Signal is evaluated on bar t and entry is the next bar OPEN (t+1).
    - No future data is used for stop/target selection beyond levels already
      confirmed on the signal bar.
    - Detailed diagnostics explain why signals/entries were skipped.
    """
    empty = {
        "trades": 0, "closed_trades": 0, "open_trades": 0,
        "win_rate": 0.0, "profit_pct": 0.0,
        "max_drawdown_pct": 0.0, "profit_factor": 0.0,
        "avg_trade_pct": 0.0,
        "bars_used": 0, "signals": 0, "entries": 0,
        "skipped_invalid_indicators": 0, "skipped_no_signal": 0,
        "skipped_bad_entry": 0, "skipped_bad_stop": 0,
        "skipped_no_target": 0
    }
    ...
    try:
        # CRITICAL FIX: backtest must have the same indicators/features as the scanner.
    ...
    # Keep enough warm-up history for EMA/rolling/pivot calculations, then test
    # only the requested number of historical bars.
    warmup = max(BACKTEST_WARMUP, 220)
    required_history = warmup + max(int(max_bars), 1) + 2
    data = bt.tail(required_history).copy()
    empty["bars_used"] = max(0, len(data) - warmup)
    ...
    diagnostics["bars_used"] = len(data) - warmup

    for i in range(warmup, len(data) - 2):
        ...
        needed = [
            ...
            "ema20", "ema50", "rsi", "macd", "macd_signal", "atr",
            "volume_ratio", "body_pct", "atr_pct"
        ]
        try:
            valid = all(np.isfinite(float(row.get(c, np.nan))) for c in needed)
        except Exception:
            valid = False
        if not valid:
            diagnostics["skipped_invalid_indicators"] += 1
            continue
        ...
        # Use the same broad signal families as the scanner, but evaluated only
        # from information available on the signal candle.
        try:
        ...
        # Fallback signal logic protects the backtest from depending solely on
        # a boolean column if a dataframe was produced by an older indicator set.
        breakout_fallback = (
            np.isfinite(prev_res) and close > prev_res and
            vol_ratio >= 1.30 and body_pct >= 0.35 and
            close > ema20 > ema50 and
            0.015 <= atr_pct <= 0.12
        )
        pullback_fallback = (
            close > ema20 > ema50 and
            (abs(close - ema20) / close <= 0.03 or abs(close - ema50) / close <= 0.05) and
            rsi >= 45 and macd >= macd_signal
        )

        signal = breakout or pullback or immediate or breakout_fallback or pullback_fallback
        if not signal:
            diagnostics["skipped_no_signal"] += 1
            continue

        # Entry = next candle OPEN. Never use its close/high/low for the entry.
        entry_i = i + 1
        try:
            entry = float(data.iloc[entry_i]["Open"])
        except Exception:
            entry = np.nan

        if not np.isfinite(entry) or entry <= 0:
            diagnostics["skipped_bad_entry"] += 1
            continue
        diagnostics["entries"] += 1

        # Stop uses signal-bar confirmed swing low when available; ATR is a
        # mechanical fallback so a valid signal is not silently discarded.
        try:
            swing_low = float(row.get("confirmed_swing_low", np.nan))
        except Exception:
            swing_low = np.nan

        if np.isfinite(swing_low) and 0 < swing_low < entry:
            stop = swing_low - atr_val * 0.20
            # Prevent a structural stop from being absurdly far away.
            min_stop = entry - atr_val * 1.50
            stop = max(stop, min_stop)
        else:
            stop = entry - atr_val * 1.50

        if not np.isfinite(stop) or stop <= 0 or stop >= entry:
            diagnostics["skipped_bad_stop"] += 1
            continue

        # Structural targets known on the signal bar only.
        target_candidates = []
        target_columns = [
            ("previous_resistance", "مقاومة سابقة"),
            ("resistance", "مقاومة"),
            ("confirmed_swing_high", "آخر Swing High مؤكد"),
            ("fib_ext_1272", "Fib 127.2%"),
            ("fib_ext_1618", "Fib 161.8%"),
            ("fib_ext_2000", "Fib 200%")
        ]
        for col, _name in target_columns:
            try:
                x = float(row.get(col, np.nan))
                if np.isfinite(x) and x > entry * 1.001:
                    target_candidates.append(x)
            except Exception:
                pass

        target_candidates = sorted(set(round(x, 8) for x in target_candidates))

        # If a structural target is not available, use a confirmed ATR-sized
        # technical objective ONLY as a last-resort backtest target. This keeps
        # the backtest from reporting zero merely because a structural level is
        # absent, while the live scanner can continue to enforce its own target
        # policy. The fallback is explicitly tagged in the trade record.
        target_source = "structural"
        if target_candidates:
            target = target_candidates[0]
        else:
            target = entry + atr_val * 2.0
            target_source = "ATR fallback"

        if not np.isfinite(target) or target <= entry:
            diagnostics["skipped_no_target"] += 1
            continue
        ...
        for j in range(entry_i, len(data)):
            bar = data.iloc[j]
            try:
                low = float(bar["Low"])
                high = float(bar["High"])
            ...
            # Conservative assumption: if both are touched in the same OHLC
            # candle, stop is assumed to have happened first.
            if low <= stop:
                outcome, exit_price, exit_i = "loss", stop, j
                break
            if high >= target:
                outcome, exit_price, exit_i = "win", target, j
                break
        ...
        if outcome != "open":
            equity *= (1.0 + ret)
        mark_equity = equity if outcome != "open" else equity * (1.0 + ret)
        peak = max(peak, mark_equity)
        max_dd = max(max_dd, (peak - mark_equity) / peak)
        ...
        next_allowed_i = max(next_allowed_i, int(exit_i) + 1)
        if exit_i >= len(data) - 1:
            break
    ...
    diagnostics.update({
        ...
        "profit_pct": (equity - 1.0) * 100,
        "max_drawdown_pct": max_dd * 100,
        "profit_factor": (gross_win / gross_loss) if gross_loss > 0 else (999.0 if gross_win > 0 else 0.0),
        "avg_trade_pct": float(np.mean([t["return"] for t in closed]) * 100) if closed else 0.0
    })
    return diagnostics
...
def process(
    ...
    try:
        ...
        if run_backtest:
            bt = run_real_backtest(df_d, backtest_bars)
            ...
            result["Backtest Invalid Indicators"] = int(bt.get("skipped_invalid_indicators", 0))
        else:
