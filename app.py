...
def run_real_backtest(df, max_bars=300):
    """Event-driven long-only backtest with strict no-lookahead rules.

    Signal is evaluated on bar t, entry is the next bar open (t+1).
    Structural stop/target levels are frozen from the signal bar only.
    Trades never overlap. If stop and target are both touched in one OHLC bar,
    stop is conservatively assumed first. Open trades are excluded from the
    closed-trade win-rate/profit-factor counts but are marked-to-market at the
    last close for the equity curve.
    """
    empty = {
    ...
    if df is None or df.empty or len(df) < BACKTEST_WARMUP + 30:
        return empty

    data = df.tail(max_bars + BACKTEST_WARMUP + 2).copy()
    trades = []
    equity = 1.0
    peak = 1.0
    max_dd = 0.0
    next_allowed_i = BACKTEST_WARMUP

    for i in range(BACKTEST_WARMUP, len(data) - 2):
        # No overlapping positions: skip signal evaluation while a prior trade is open.
        if i < next_allowed_i:
            continue

        row = data.iloc[i]
        needed = ["Close", "ema20", "ema50", "rsi", "macd", "macd_signal", "atr"]
        try:
        ...
        breakout = (
            ...
            vol_ratio >= 1.5 and body_pct >= 0.45 and
            close > float(row["ema20"]) > float(row["ema50"])
        )
        pullback = bool(row.get("pullback", False)) and float(row["ema20"]) > float(row["ema50"])
        immediate = (
            close > float(row["ema20"]) > float(row["ema50"]) and
            float(row["rsi"]) >= 50 and float(row["macd"]) > float(row["macd_signal"]) and
            vol_ratio >= 1.0
        )
        if not (breakout or pullback or immediate):
            continue

        # Entry is always the next candle open; never use future close/high/low to set it.
        entry_i = i + 1
        entry = float(data.iloc[entry_i]["Open"])
        if not np.isfinite(entry) or entry <= 0:
            continue

        # Stop is structural first, ATR fallback second, using signal-bar information only.
        try:
            swing_low = float(row.get("confirmed_swing_low", np.nan))
        except Exception:
            swing_low = np.nan
        if np.isfinite(swing_low) and 0 < swing_low < entry:
            stop = max(swing_low - atr_val * 0.20, entry - atr_val * 1.50)
        else:
            stop = entry - atr_val * 1.50
        if not np.isfinite(stop) or stop <= 0 or stop >= entry:
            continue

        # Structural targets only. Missing targets are not invented.
        target_candidates = []
        for col in [
            "previous_resistance", "resistance", "confirmed_swing_high",
            "fib_ext_1272", "fib_ext_1618", "fib_ext_2000"
        ]:
            try:
                x = float(row.get(col, np.nan))
                if np.isfinite(x) and x > entry:
        ...
        for j in range(entry_i + 1, len(data)):
            ...
            low = float(bar["Low"])
            high = float(bar["High"])
            # Conservative OHLC assumption when both levels are touched in one candle.
            if low <= stop:
        ...
        # Closed trades compound equity. An unfinished trade is marked-to-market only.
        if outcome != "open":
            equity *= (1.0 + ret)
        ...
        trades.append({
            "return": float(ret), "outcome": outcome,
            "entry": float(entry), "exit": float(exit_price),
            "bars": int(exit_i - entry_i)
        })

        # Freeze the scanner until this position exits; this prevents overlapping trades.
    ...
    avg_trade = float(np.mean([t["return"] for t in closed]) * 100) if closed else 0.0
