...
def build_market_return_proxy(data, symbols, lookback=20):
    """
    يبني عائد سوق عرضي (Cross-sectional EGX proxy) لآخر فترة متاحة.

    الأولوية للبيانات الفعلية للأسهم الموجودة في universe. لكل سهم نحسب
    العائد من إغلاق آخر شمعة إلى إغلاق lookback شمعة سابقة، ثم نستخدم
    الوسيط Median لتقليل تأثير الأسهم الشاذة أو الأسهم ضعيفة السيولة.

    يرجع 0.0 عند عدم كفاية البيانات بدلًا من كسر التطبيق.
    """
    try:
        if data is None or data.empty or not symbols:
            return 0.0

        returns = []
        for symbol in symbols:
            df = extract_symbol_data(data, symbol)
            ...
            if prev <= 0 or not np.isfinite(last) or not np.isfinite(prev):
                continue
        ...
        # Median is more robust than mean against extreme EGX movers.
